% Exports the actuator geometry to the database

% Exports the geometry in actGeometry to the database as acutator geometry.
% The user is asked for the height.  actuator_geometry_id is changed to the
% current actuator_geometry_id in the database.  It is not changed if there
...
height = str2double(input('Input actuator height: ','s'))

% Begin export of geometry
fprintf('Exporting actuator geometry ...\n');

geometryData.is_object = 0;
geometryData.geometry = MatlabToPostgresqlArray( actGeometry );
geometryData.height = height;

geometryFields = {'is_object','geometry','height'};
...
actuator_geometry_id = cell2mat(fetch(dbConn, 'SELECT max(object_actuator_geometry_id) FROM object_actuator_geometry WHERE is_object = 0;'));
...
fprintf('Exporting actuator geometry completed\n');
