function contactMode = collisionDetectionRound(aPosCtrl, objRadius)
...
RB1 = objRadius+5;
RB2 = sum(sqrt(B2(:, 1).*B2(:, 1) + B2(:, 2).*B2(:, 2)))/size(B2, 1);
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    fContact(1) = circleCircleTest(fixels(1, :), Pos(:)');
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    fContact(2) = circleCircleTest(fixels(2, :), Pos(:)');
end
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    fContact(3) = circleCircleTest(fixels(3, :), Pos(:)');
end
...
% Each fixel is assigned a power of 2 (1, 2 and 4).
% vThe contact mode is determined by finding the sum of the fixels that are in contact with the fixels.
...
function collide = polyCircleTest(xa, xo)
%only detect once with p1 = act, p2 = body
global B2
global epsilon
global objR
...
for j=1:n2
    e1 = j;
    e2 = bmod(j+1, n2);
    e1W = transformFrame(b2(e1, :), xa);
    e2W = transformFrame(b2(e2, :), xa);
    [dis isE1 isE2] = dis2Segment( xo(1:2), e1W, e2W ); %measure the distance from the circle center to the body segment
    if dis <= radius + epsilon
        ...
        if isE1
            e = e1W - xo(1:2);
            e = e/norm(e, 2);
            addCircleResult(dis-radius, e1W, xo(1:2) + radius * e, 'actuator', xo(1:2) - e1W);
        elseif isE2
            e = e2W - xo(1:2);
            e = e/norm(e, 2);
            addCircleResult(dis-radius, e2W, xo(1:2) + radius * e, 'actuator', xo(1:2) - e2W);
        else
            [ regDis pLoc ] = vertLineDist(xo(1:2), e1W, e2W);
            e = pLoc(:)' - xo(1:2);
            e = e/norm(e, 2);
            addCircleResult(dis-radius, pLoc, xo(1:2) + radius * e, 'actuator', xo(1:2) - pLoc);
        end
    end
end

function collide = circleCircleTest(xp, xo)
...
dis = norm(xp(1:2) - xo(1:2), 2);
e = (xp(1:2) - xo(1:2)) / dis;
if dis <= pegR + objR + epsilon
    collide = true;
    locObj = xo(1:2) + e * objR;
    locPeg = xp(1:2) - e * pegR;
    addCircleResult(dis - pegR - objR, locPeg, locObj, 'peg', xo(1:2)-locObj);
end
...
% record a collision result
function [] = addCircleResult(d, LocCon, Loc, body, e)
global m_numContacts colRes
m_numContacts = m_numContacts + 1;

col.body = body;
col.distance = d;
col.contactLoc = LocCon;
col.vertex = Loc;
