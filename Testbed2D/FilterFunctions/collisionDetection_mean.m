function [cM tM] = collisionDetection_mean(aPosCtrl, objData, actGeometry, fixels, pegR)
...
RB1 = max(sqrt(B1(:, 1).*B1(:, 1) + B1(:, 2).*B1(:, 2))) + 5;%/size(B1, 1);
RB2 = max(sqrt(B2(:, 1).*B2(:, 1) + B2(:, 2).*B2(:, 2))) + 5;%/size(B2, 1);
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    [fContact(1) dis1 tfContact(1)] = polyCircleTest(fixels(1, :), Pos, pegR);
    if DEBUG
        disp(['fixel1: ']);
        dis1
    end
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    [fContact(2) dis2 tfContact(2)] = polyCircleTest(fixels(2, :), Pos, pegR);
    if DEBUG
        disp(['fixel2: ']);
        dis2
    end
...
if circleIntersectTest(Pos(1), Pos(2), RB1, ...
    ...
    [fContact(3) dis3 tfContact(3)] = polyCircleTest(fixels(3, :), Pos, pegR);
    if DEBUG
        disp(['fixel3: ']);
        dis3
    end
...
cM = fContact(1) * 4 + fContact(2) * 2 + fContact(3) * 1;
tM = tfContact(1) * 4 + tfContact(2) * 2 + tfContact(3) * 1;
return
...
function [collide shortDis touch] = polyCircleTest(x1, x2, pegR)
