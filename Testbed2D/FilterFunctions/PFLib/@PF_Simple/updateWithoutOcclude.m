function [fltr]  = updateWithoutOcclude(fltr, aPosCtrl, kFV)
...
parfor i = 1:N
    % propagate particles
    if ~closure(i)
        [p(:, i) para(:, i) contactMode(i) closure(i) touchMode(i)] = f(p(:, i), aPosCtrl, frameRate, objData, B2, fCenter, pR, para0, wPara(:, i), wPext(:, i));
    end
    ...
    % update weights
    wtUpdate =  coefficientUpdate(yTraj, y_l, v_d, yTac, touchMode(i), closure(i), kFV);
