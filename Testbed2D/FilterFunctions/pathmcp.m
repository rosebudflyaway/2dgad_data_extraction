function [z,status,f,J,mu] = pathmcp(z,l,u,cpfj,A,b,t,mu)
% pathmcp(z,l,u,cpfj,A,b,t,mu)
%
% Solve a polyhedrally constrained variational inequality using PATH
%
% Calling syntax: [z,f,J] = pathmcp(z,l,u,cpfunjac_pf,A,b,t,mu)
%
% Input:
%  z - starting point
%  l - lower bounds on z
%  u - upper bounds on z
%
%  cpfunjac_pf - the name of the m-file for  evaluating the function F and its 
%             Jacobian J (without .m-extension).
%
%             The following m-file must be supplied (where default name is
%             'mcp_funjac.m' unless stated otherwise in the variable cpfunjac_pf).
%
%             'mcp_funjac.m' contains function [f,J,domerr]=cpfunjac_pf(z,jacflag)
...
if (status ~= 1) 
 % status,
%  disp('Path fails to solve problem');
  z = 0;
  J = 0;
  f = 0;
  mu = 0;
