function [x_lp1 para cM closure tM] = ffun_mean(x_l, aPosCtrl, FRAMERATE, objData, actGeometry, fixels, pegR, para0)
%ffun_pf system dynamic model, implement one time step of the dynamic system
%
%Synopsis:
%      [x_lp1 para contactMode closure] = ffun_mean(x_l, aPosCtrl, para0,
%      wPara, wPext)
%
%Input:
...
if objData.is_circular
    [cM tM ] = collisionDetectionRound_mean(aPosCtrl, objData, actGeometry, fixels, pegR); % generate col_res and m_numContacts and set isInCollision
else
	[cM tM] = collisionDetection_mean(aPosCtrl, objData, actGeometry, fixels, pegR); % generate col_res and m_numContacts and set isInCollision
end
